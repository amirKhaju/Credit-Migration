function VaR_ex = compute_VaR_for_rho(rho,transition_matrix,df_1y2ydef,df_expiry,bond_mtm_A,bond_mtm_BBB,frwdis6mesi,face_value,recovery_rate,mc_simulations,issuers_num_A,issuers_num_BBB,seed)
%
...
N = issuers_num_A + issuers_num_BBB;  % total number of bonds (100)
...
lossexd_BBB = bond_mtm_A / df_expiry(3) - price1y_BBB;

lossexd=[lossexd_A; lossexd_BBB];
...
idx = ceil(0.001 * mc_simulations) - 1;    % maybe there is no the -1
% idx = ceil(0.001 * mc_simulations);

% Compute VaR
VaR_ex = lossordexd(idx) * (issuers_num_A+issuers_num_BBB);
