...
df_1y2ydef_CCC = (1-pd_1y_BBB)*frwdisc1y2y+pd_1y_BBB*df_expiry(4)/df_expiry(3)*recovery_rate;
...
%%
rho_vec = ones(100,1)*calibrated_rho; % [100x1] vettore con correlazioni
ratingInit = [repmat(rating_A,50,1); repmat(rating_BBB,50,1)]; % 50 A (3), 50 BBB (4)
...
VaR = compute_VaR_for_rho(calibrated_rho,transition_matrix,df_1y2ydef,df_expiry,bond_mtm_A,bond_mtm_BBB,frwdis6mesi,face_value,recovery_rate,mc_simulations,issuers_num_A,issuers_num_BBB,seed)
